...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname preprova1t2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
;  (local
    (define(idade-media no)
         (cond
           [(empty? no) 0]
           [(and(empty? (pessoa-pai no))(empty? (pessoa-mae no))) ( - 2015 (pessoa-ano no))]
           [else (calculo-media no)]))
       
       (define (calculo-media no)( / (soma-idades no) (total-parentes no)))
